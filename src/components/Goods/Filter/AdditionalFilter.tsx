...
const AdditionalFilter: React.FC = () => {
  ...
  const [activeAdditionalFilter, setActiveAdditionalFilter] = useState<
    ...
    if (key !== null && value !== null) {
      ...
      dispatch(
        setFilter({
          min: updatedMin,
          max: updatedMax,
          updatedFilter: activeAdditionalFilter,
        })
      );
    }
  };
    ...
    <>
      <div className="additional-filter price-filter">
        ...
        <input
          ...
          onChange={(e) =>
            setSelectPrice({ ...selectPrice, min: e.target.value })
          }
        />
        <span> to </span>
        <input
          ...
          onChange={(e) =>
            setSelectPrice({ ...selectPrice, max: e.target.value })
          }
        />
        <button
          onClick={() =>
            handleCheckboxChange(null, null, selectPrice.min, selectPrice.max)
          }
        >
          Apply
        </button>
      ...
      {additionalFilter.map((key, index) => {
        ...
        return (
          <div className="additional-filter" key={index}>
            <button
              className={isActive ? "button-active" : "button-default"}
