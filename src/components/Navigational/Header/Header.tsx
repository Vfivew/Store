...
import england from "../../../assets/img/icon/england.svg";
import ukraine from "../../../assets/img/icon/ukraine.svg";
...
const Header = () => {
  ...
  const burgerActive = useAppSelector(
    (state) => state.media.headerBurgerActive
  );
  ...
  const {
    ...
    isError: basketError,
  } = useFetchBasketQuery(email);
  const {
    data: desireData,
    isLoading: desireLoading,
    isError: desireError,
  } = useFetchDesireQuery(email);
  const countItem = basketItems.length;
  const countDesireItem = desireItems.length;
  const arrayBasketData = basketData
    ? Object.entries(basketData).map(([key, value]) => value)
    : [];
  const arrayDesireData = desireData
    ? Object.entries(desireData).map(([key, value]) => value)
    : [];
  ...
  return (
    <header className="header">
      <section className="header-nav">
        ...
        <div
          className={`header-list-container ${burgerActive ? "active" : ""}`}
        >
