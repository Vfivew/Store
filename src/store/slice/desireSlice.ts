...
import { DesireState } from "../../types/types";
...
const desireSlice = createSlice({
  ...
  reducers: {
    ...
    removeDesireItem: (state, action) => {
      ...
      state.desire = state.desire.filter(
        (item) => item[0].article !== itemIdToRemove
      );
    },
  },
});

export const { setDesireItem, addDesireItem, removeDesireItem } =
  desireSlice.actions;
